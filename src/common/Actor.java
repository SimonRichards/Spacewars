...
public abstract class Actor {
     ...
     */
    protected Actor(int actorID, Vector2d initPos, Vector2d initV) {
        this(actorID);
     ...
     */
    protected Actor(int actorID, double[] buffer) {
        this(actorID);
...
//      velocity = new Vector2d(buffer[3], buffer[4]);
    ...
    public void gravitate(Actor other) {
        ...
        Vector2d myAccelVec = new Vector2d(grav_vector);
        myAccelVec.scale(my_accel);
        this.accelerate(myAccelVec);

        Vector2d otherAccelVec = new Vector2d(grav_vector);
     ...
     * range 0 <= angle <= 2*PI
     */
    public static double angleWraparound(double angle) {
        double theta = angle;
        while (theta < -Math.PI) {
            theta += 2.0 * Math.PI;
        }
    ...
    public enum ActorType {
         ...
         */
        public static ActorType fromInt(int index) {
     ...
     * @return A new actor
     */
    public static Actor fromBuffer(ActorType type, int actorID, double[] buffer) {
        switch (type) {
            case NEEDLE:
                return new Spacecraft.Needle(buffer);
            case WEDGE:
                return new Spacecraft.Wedge(actorID, buffer);
            case STAR:
                return new Star(buffer);
            case MISSILE:
                return new Missile(actorID, buffer);
            default:
